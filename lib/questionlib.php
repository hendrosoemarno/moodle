...
// Tambahkan kode untuk menangani file questiontext tanpa login
if ($filearea === 'questiontext') {
    ...
    // Opsional: Batasi akses ke konteks tertentu (misalnya, konteks sistem atau kategori publik)
    $allowed_contexts = [1]; // Ganti dengan ID konteks yang diizinkan (misalnya, konteks sistem)
    if ($file && !$file->is_directory() && in_array($context->id, $allowed_contexts)) {
        send_stored_file($file, 0, 0, $forcedownload, $options);
        return;
    }
}

        // Coba fallback jika file questiontext tidak ditangani plugin manapun.
        if (global_questiontext_pluginfile($context, $component, $filearea, $args, $forcedownload, $options)) {
            return;
        }
...
function global_questiontext_pluginfile($context, $component, $filearea, $args, $forcedownload, $options) {
    if (!isloggedin()) {
        return false;
    }
