...
class service_provider implements \core_payment\local\callback\service_provider {
    ...
    public static function get_payable(string $paymentarea, int $instanceid): \core_payment\local\entities\payable {
        ...
        return new \core_payment\local\entities\payable($instance->cost, $instance->currency, $instance->customint1, $instance->name);
    }
    
    public static function get_template_context(string $paymentarea, int $instanceid): array {
        global $DB;

        $instance = $DB->get_record('enrol', ['enrol' => 'fee', 'id' => $instanceid], '*', MUST_EXIST);

        return [
            'cost' => format_price($instance->cost, $instance->currency),
            'instanceid' => $instance->id,
            'instancename' => $instance->name, // Tambahkan instancename dari $instance->name
            'currency' => $instance->currency,
            'successurl' => (new moodle_url('/course/view.php', ['id' => $instance->courseid]))->out(),
