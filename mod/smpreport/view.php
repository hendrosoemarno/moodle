<?php
// File: mod/smpreport/view.php

require_once(__DIR__ . '/../../config.php');

$id = required_param('id', PARAM_INT); // ID dari course module.
$cm = get_coursemodule_from_id('smpreport', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', ['id' => $cm->course], '*', MUST_EXIST);
$smpreport = $DB->get_record('smpreport', ['id' => $cm->instance], '*', MUST_EXIST);

require_login($course, true, $cm);
$context = context_module::instance($cm->id);
require_capability('mod/smpreport:view', $context);

$PAGE->set_url('/mod/smpreport/view.php', ['id' => $id]);
$PAGE->set_title(format_string($smpreport->name));
$PAGE->set_heading(format_string($course->fullname));

echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($smpreport->name));
