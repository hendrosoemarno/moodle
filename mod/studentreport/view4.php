...
$render_assessment_legend = function() {
    ...
    echo html_writer::tag('div',
        ...
        ['class' => 'box generalbox', 'style' => 'max-width:720px;margin-bottom:1rem;']
    );
};

$render_assessment_legend(); // Tampilkan keterangan penilaian di bagian atas.
...
/** Jika tidak ada data, tetap tampilkan legend di atas, lalu info kosong */
...
/** =======================================================================
 *  1) Ambil nama kategori asli (qc.name) via qc_id (1 query batch)
 *  =======================================================================
 */
$qcids = [];
...
/** =======================================================================
 *  2) Deteksi subject berdasarkan tag 'bin' / 'mat' pada nama kategori asli
 *  =======================================================================
 */
$detect_subject_by_tag = function(string $text): ?string {
    // pencarian token 'bin' / 'mat' (case-insensitive), boleh diapit spasi/strip/kurung
...
foreach ($raw_records as $rec) {
    // Ambil nama kategori penuh dari qcmap; fallback ke category_name dari view bila tidak ada.
    $fullcat = $qcmap[$rec->qc_id] ?? $rec->category_name;

    $subject = $detect_subject_by_tag($fullcat);
    if ($subject === null) {
        // fallback opsional: coba deteksi dari nama kuis
...
/** =======================================================================
 *  3) Fungsi pivot + render tabel per subject
 *  =======================================================================
 */
$render_subject_table = function(string $subject, array $subject_records) use ($OUTPUT) {
    ...
    foreach ($subject_records as $record) {
        ...
        // Simpan total & correct untuk kategori (pakai category_name dari view agar header rapi)
